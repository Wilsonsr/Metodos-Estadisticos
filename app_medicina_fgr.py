...
st.markdown(f"""
**¿Qué estoy viendo?**  
- **Matriculados**: estudiantes que entran cada año.  
- **Graduados**: egresan 6 años después de su matrícula.  
...
# ------------------------------------------------------------
# Gráfico principal (limpio, con opción de detalles)
# ------------------------------------------------------------
fig = go.Figure()

# Trazas principales (colores legibles y hover unificado)
fig.add_trace(go.Scatter(
    x=df['Anio'], y=df['Matriculados'],
    mode='lines+markers', name='Matriculados',
...
fig.add_trace(go.Scatter(
    ...
    mode='lines+markers', name='Graduados',
...
# Línea vertical y etiqueta para 2031 (siempre)
...
# Resumen de diferencia vs HW (solo si hay HW y se muestran detalles)
if mostrar_detalle and df_hw['Matriculados_HW'].notna().any() and not df_dif.empty:
    fig.add_annotation(
        x=2027.5, y=y_max * 0.98,
        text=f"Matriculados vs HW (2026–2029): {diferencia_total:+,}",
        showarrow=False, bgcolor="white", bordercolor="#d62728", borderwidth=1
    )
...
# ------------------------------------------------------------
# Densidad lograda (por 1.000) + franja objetivo
# ------------------------------------------------------------
...
fig_den.add_trace(go.Scatter(
    ...
    hovertemplate="Año %{x}<br>Densidad: %{y:.2f}<extra></extra>"
))

# Franja de meta ± 0.05
fig_den.add_hrect(y0=densidad_objetivo-0.05, y1=densidad_objetivo+0.05,
                  fillcolor="lightgreen", opacity=0.15, line_width=0,
                  annotation_text="Rango objetivo", annotation_position="top left")
